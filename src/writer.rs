...
impl<T: Write> Writer<T>{
    ...
    pub fn write(&mut self, event: &Event) -> Result<(), io::Error> {
        ...
        for vertex in &event.vertices {
            ...
            let particles = vertex.particles_in.iter()
    ...
    fn write_event_line(&mut self, event: &Event) -> Result<(), io::Error> {
        write!(
            self.stream,
            "E {} {} {} {} {} {} {} {} 0 0 {}",
            event.number, event.mpi, event.scale, event.alpha_qcd,
            event.alpha_qed, event.signal_process_id, event.signal_process_vertex,
            event.vertices.len(), event.random_states.len()
        )?;
    ...
    fn write_vertex_line(&mut self, vertex: &Vertex) -> Result<(), io::Error> {
        write!(
            self.stream,
            "V {} {} {} {} {} {} 0 {} {}",
            vertex.barcode, vertex.status,
            vertex.x, vertex.y, vertex.z, vertex.t,
    ...
    fn write_particle_line(&mut self, particle: &Particle) -> Result<(), io::Error> {
        write!(
            ...
            "P 0 {} {} {} {} {} {} {} {} {} {} {}",
            particle.id,
            particle.p[1], particle.p[2], particle.p[3], particle.p[0],
            particle.m, particle.status, particle.theta, particle.phi,
            particle.end_vtx, particle.flows.len()
        )?;
    ...
    fn write_cross_section_line(&mut self, xs: &CrossSection) -> Result<(), io::Error> {
        writeln!(self.stream, "C {} {}", xs.cross_section, xs.cross_section_error)
    }
    ...
    fn write_pdf_info_line(&mut self, pdf: &PdfInfo) -> Result<(), io::Error> {
        writeln!(
            self.stream,
            "F {} {} {} {} {} {} {} {} {}",
            pdf.parton_id[0], pdf.parton_id[1],
            pdf.x[0], pdf.x[1],
            pdf.scale,
            pdf.xf[0], pdf.xf[1],
            pdf.pdf_id[0], pdf.pdf_id[1],
        )
    }
